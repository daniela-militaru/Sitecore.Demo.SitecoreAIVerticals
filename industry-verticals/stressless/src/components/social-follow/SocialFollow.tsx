      ...
      <div className="flex flex-col gap-y-4">
        {socialLinks.map(({ icon, field, key }) => (
          <div key={key} className="flex items-center gap-2">
            {field?.value?.href && (
              <>
                <FontAwesomeIcon icon={icon} className="text-xl text-gray-200" />
                <ContentSdkLink field={field} className="text-gray-200 hover:underline" />
              </>
            )}
          </div>
        ))}
      ...
      <div className="flex gap-2">
        {socialLinks.map(({ icon: Icon, field, key }) => (
          <div key={key} className="flex items-center gap-4">
            <ContentSdkLink field={field} className="text-gray-200 hover:text-white">
              <Icon />
            </ContentSdkLink>
          </div>
        ))}
